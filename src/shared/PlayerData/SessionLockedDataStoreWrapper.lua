...
--Uses the keyInfo provided in the DataStore:UpdateAsync callback to determine whether the key 
--is safe to update, based on the wrapper’s knowledge of the locks it maintains and the lockSettings.
...
-- Removes the lock iden_requestAsynctifier from the metadata table
...
function SessionLockedDataStoreWrapper.refreshLockAsync(self: ClassType, key: string): (boolean, string?)
	return self:_requestAsync(key, function(value: any, keyInfo: DataStoreKeyInfo)
		local userIds = if keyInfo then keyInfo:GetUserIds() else nil :: { number }?
		local metadata = if keyInfo then keyInfo:GetMetadata() else nil
...
function SessionLockedDataStoreWrapper.getAsync(
	...
	return self:_requestAsync(key, function(value: any, keyInfo: DataStoreKeyInfo)
		...
		if userIdsToInclude then
			userIds = mergeArraysUniqueOnly(oldUserIds, userIdsToInclude) :: { number }
		end
